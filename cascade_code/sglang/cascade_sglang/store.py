...
def page_key(
    ...
    *,
    ...
    model = "-".join((model_name or "").split("/"))
    if is_mla:
        return f"{NAMESPACE}-{model}-{page_hash}"
    return f"{NAMESPACE}-{model}-tp{tp_rank}of{tp_size}-{page_hash}"
...
class _CascadeNodeServer:
    ...
    def __init__(
        ...
        from cascade_sglang.shm_pool import SegmentMapper

        self._segment_mapper = SegmentMapper()
    ...
    def _dispatch(self, request):
        ...
        if op == "prefetch":
            ...
            return len(fresh)
        if op in ("put_shm_batch", "get_shm_batch"):
            from cascade_sglang.shm_pool import PoolGeometry

            geometry = PoolGeometry(**request["geometry"])
            views = []
            for index in request["host_indices"]:
                views.extend(self._segment_mapper.page_views(geometry, int(index)))
            arrays = [
                np.frombuffer(view, dtype=np.uint8) for view in views
            ]
            if op == "put_shm_batch":
                flags = [True] * len(request["keys"])
                with self._native_write_lock():
                    stored = int(
                        self.native._store.put_batch(request["keys"], arrays, flags)
                    )
                return stored
            self._ensure_metadata_sync()
            with self._native_read_lock():
                _, sizes = self.native._store.get_batch(request["keys"], arrays)
            return [int(size) for size in sizes]
        if op == "available_prefix":
            self._ensure_metadata_sync()
            keys = request["keys"]
            with self._native_read_lock():
                return int(self.native._store.available_prefix(keys))
...
class _CascadeNodeClient:
    ...
    def __init__(self, socket_path: Path,
                 timeout_s: float = float(os.environ.get("CASCADE_RPC_TIMEOUT", "30"))):
        self.socket_path = socket_path
...
class CascadeIndexClient:
    ...
    def _candidate_sockets() -> list[Path]:
        ...
        if namespace:
            for path in sorted(Path("/tmp").glob(f"cascade-sglang-{namespace}-*.sock")):
                if path != preferred:
...
class CascadeStore:
    ...
    def __init__(self, extra: dict[str, Any]):
        ...
        set_if_present("aggregated_lustre", bool(extra.get("aggregated_lustre", False)))
    ...
    def _rpc_socket_path() -> Path:
        ...
        return Path(f"/tmp/cascade-sglang-{namespace}-{node_rank}.sock")
    ...
    def put_pages(self, keys: list[str], geometry, host_indices) -> int:
        request = {
            "op": "put_shm_batch",
            "keys": list(keys),
            "geometry": geometry.as_dict(),
            "host_indices": [int(i) for i in host_indices],
        }
        if self._client is not None:
            return int(self._rpc_call(request))
        if self._server is not None:
            return int(self._server._dispatch(request))
        raise RuntimeError("Cascade page path needs the node server or a client")

    def get_pages(self, keys: list[str], geometry, host_indices) -> list[int]:
        request = {
            "op": "get_shm_batch",
            "keys": list(keys),
            "geometry": geometry.as_dict(),
        ...
        if self._client is not None:
            return [int(n) for n in self._rpc_call(request)]
        if self._server is not None:
            return [int(n) for n in self._server._dispatch(request)]
        raise RuntimeError("Cascade page path needs the node server or a client")

...
bool GPUBackend::put_device(const BlockId &id, const void *data, size_t size) {
  if (index_.contains(id)) {
    return true;
  }
  if (used_.load() + size > capacity_) {
  ...
  cudaError_t err = cudaMemcpyAsync(gpu_ptr, data, size,
                                    cudaMemcpyDefault, stream);
  if (err != cudaSuccess) {
    free_gpu(gpu_ptr, size);
    return false;
  }
...
size_t GPUBackend::put_device_batch(
  ...
  for (size_t i = 0; i < ids.size(); ++i) {
    if (index_.contains(ids[i])) continue;
    if (used_.load() + sizes[i] > capacity_) continue;

    void *gpu_ptr = alloc_gpu(sizes[i]);
    if (!gpu_ptr) continue;
    ...
    int stream_id = tid % NUM_STREAMS;
    cudaStream_t stream = static_cast<cudaStream_t>(cuda_streams_[stream_id]);
    cudaError_t err = cudaMemcpyAsync(gpu_ptr, data[i], sizes[i],
                                      cudaMemcpyDefault, stream);
    if (err != cudaSuccess) {
      free_gpu(gpu_ptr, sizes[i]);
      continue;
    }

    GPUBlock block{gpu_ptr, sizes[i]};
    index_.put(ids[i], block, sizes[i]);
    used_ += sizes[i];
    ++queued;
  }
...
bool GPUBackend::get_device(const BlockId &id, void *out_data,
  ...
  *out_size = block.size;
  ...
  cudaError_t attr_err = cudaPointerGetAttributes(&out_attrs, out_data);
  if (attr_err == cudaSuccess && out_attrs.type == cudaMemoryTypeDevice &&
      out_attrs.device == device_id_) {
    kind = cudaMemcpyDeviceToDevice;
  }
  cudaError_t err = cudaMemcpyAsync(out_data, block.ptr, block.size, kind,
                                    stream);
  if (err != cudaSuccess) {
    return false;
  }
...
bool GPUBackend::get_device_async(const BlockId &id, void *out_data,
                                 size_t out_capacity, size_t *out_size,
                                 void *stream) {
  auto block_opt = index_.get(id);
  if (!block_opt) {
  ...
  *out_size = block.size;
  ...
  return cudaMemcpyAsync(out_data, block.ptr, block.size, cudaMemcpyDefault,
                         static_cast<cudaStream_t>(stream)) == cudaSuccess;
}

const void *GPUBackend::device_ptr(const BlockId &id, size_t *out_size) const {
  auto block_opt = index_.get(id);
  if (!block_opt) return nullptr;
  if (out_size) *out_size = block_opt->size;
  return block_opt->ptr;
}

...
AggregatedLustreBackend::AggregatedLustreBackend(const std::string &path,
  ...
  if (const char *off = std::getenv("CASCADE_ASYNC_LUSTRE")) {
    async_ = std::string(off) != "0";
  }
  if (const char *mb = std::getenv("CASCADE_LUSTRE_WRITE_MB")) {
    flush_bytes_ = static_cast<size_t>(std::atof(mb) * 1024.0 * 1024.0);
  }
  if (const char *cap = std::getenv("CASCADE_LUSTRE_QUEUE_GB")) {
    queue_cap_bytes_ = static_cast<size_t>(std::atof(cap) * 1024.0 * 1024.0
                                           * 1024.0);
  }
  if (async_) {
    writer_ = std::thread(&AggregatedLustreBackend::writer_loop, this);
  }
}

void AggregatedLustreBackend::writer_loop() {
  for (;;) {
    BlockId id;
    std::shared_ptr<std::vector<uint8_t>> bytes;
    {
      ...
      if (it == staging_.end()) continue;
      bytes = it->second;
    }


    write_now(id, bytes->data(), bytes->size());
    {
...
bool AggregatedLustreBackend::flush_buffer_locked() {

  if (write_buf_.empty()) return true;
  if (current_fd_ < 0) return false;
  size_t done = 0;
  while (done < write_buf_.size()) {
    ssize_t n = write(current_fd_, write_buf_.data() + done,
                      write_buf_.size() - done);
    if (n <= 0) return false;
    done += static_cast<size_t>(n);
  }
  buffer_base_ += write_buf_.size();
  write_buf_.clear();
  return true;
}
...
bool AggregatedLustreBackend::put(const BlockId &id, const uint8_t *data,
                                  size_t size) {
  if (!async_) return write_now(id, data, size);
  {
    std::shared_lock<std::shared_mutex> rlock(index_mutex_);
    if (index_.count(id) > 0) return true;
  }
  std::unique_lock<std::mutex> lock(queue_mutex_);
  if (staging_.count(id) > 0) return true;


  queue_cv_.wait(lock, [&] {
    return stopping_ || staged_bytes_ + size <= queue_cap_bytes_
           || staging_.empty();
  });
...
bool AggregatedLustreBackend::write_now(const BlockId &id,
  ...
  if (current_offset_ + size + sizeof(uint64_t) > max_file_size_) {
    if (!flush_buffer_locked()) return false;
  ...
  const uint8_t *header = reinterpret_cast<const uint8_t *>(&block_size);
  write_buf_.insert(write_buf_.end(), header, header + sizeof(uint64_t));
  write_buf_.insert(write_buf_.end(), data, data + size);
  ...
  if (write_buf_.size() >= flush_bytes_) {
    if (!flush_buffer_locked()) return false;
  }
  return true;
}
...
bool AggregatedLustreBackend::get(const BlockId &id, uint8_t *out_data,
...

  {
    ...
    auto it = staging_.find(id);
    if (it != staging_.end()) {
      std::memcpy(out_data, it->second->data(), it->second->size());
...

  {
    ...
    if (loc.file_id == current_file_id_ && loc.offset >= buffer_base_) {
      size_t at = loc.offset - buffer_base_ + sizeof(uint64_t);
      if (at + loc.size <= write_buf_.size()) {
        std::memcpy(out_data, write_buf_.data() + at, loc.size);
        *out_size = loc.size;
        return true;
      }
    }
...
void AggregatedLustreBackend::flush() {
  if (async_) {
    std::unique_lock<std::mutex> lock(queue_mutex_);
    drain_cv_.wait(lock, [this] { return staging_.empty() || stopping_; });
  }
  std::lock_guard<std::mutex> lock(write_mutex_);
  flush_buffer_locked();
  if (current_fd_ >= 0) {

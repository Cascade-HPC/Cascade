...
public:
    ...
    bool put_device(const BlockId& id, const void* data, size_t size);
    ...
    bool get_device(const BlockId& id, void* out_data, size_t out_capacity,
                    size_t* out_size);


    bool get_device_async(const BlockId& id, void* out_data,
                          size_t out_capacity, size_t* out_size,
                          void* stream);

    const void* device_ptr(const BlockId& id, size_t* out_size) const;
...
private:
    ...
    bool open_new_file();
    bool write_now(const BlockId& id, const uint8_t* data, size_t size);
    bool flush_buffer_locked();


    std::vector<uint8_t> write_buf_;
    size_t buffer_base_ = 0;
    size_t flush_bytes_ = 64ULL * 1024 * 1024;


    void writer_loop();

    bool async_ = true;
    size_t queue_cap_bytes_ = 8ULL * 1024 * 1024 * 1024;
    std::thread writer_;
    mutable std::mutex queue_mutex_;
    std::condition_variable queue_cv_;
    std::condition_variable drain_cv_;
    std::deque<BlockId> queue_;
    std::unordered_map<BlockId, std::shared_ptr<std::vector<uint8_t>>> staging_;

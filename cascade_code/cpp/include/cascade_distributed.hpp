...
struct AccessRecord {
    ...
    float ema_remote_rate = 0.0f;
    uint32_t window_remote = 0;
...
public:
    ...
    uint8_t* stage_alloc(size_t size, size_t* out_offset);
    void stage_commit(const BlockId& id, size_t offset, size_t size,
                      bool is_prefix);
    void stage_release(size_t offset, size_t size);
    uint8_t* base_ptr() const { return static_cast<uint8_t*>(dram_base_); }

    const uint8_t* local_ptr(const BlockId& id, size_t* out_size) const;
    bool get_remote(int target_rank, size_t offset, uint8_t* out, size_t size);


    bool put_remote(int target_rank, size_t offset, const uint8_t* data, size_t size);
    long long reserve_local(const BlockId& id, size_t size, bool is_prefix);


    long long reserve_on_central(size_t size);
    void init_central_allocator();
    void warm_remote();


    bool device_rma_supported();


    void pin_batch(const std::vector<BlockId>& ids, double seconds);
    void unpin_batch(const std::vector<BlockId>& ids);
    bool is_pinned(const BlockId& id) const;
    size_t pinned_count() const;

    bool get_remote_batch(const std::vector<int>& target_ranks,
                          const std::vector<size_t>& offsets,
                          const std::vector<uint8_t*>& outs,
                          const std::vector<size_t>& sizes);


    bool get_remote_device_batch(
...
private:
    ...
    std::atomic<size_t> prefix_evictions_{0};

public:
    size_t prefix_evictions() const { return prefix_evictions_.load(); }
...
private:
    ...
    std::unordered_map<BlockId, PinEntry> pinned_blocks_;


#ifdef USE_MPI
    MPI_Win alloc_win_ = MPI_WIN_NULL;
    MPI_Aint *alloc_counter_ = nullptr;
#endif

    std::atomic<int> device_rma_state_{-1};
    std::mutex device_rma_mutex_;
...
public:
    ...
    bool put_device(const BlockId& id, const void* data, size_t size, bool is_prefix = false);
    size_t put_device_batch(const std::vector<BlockId>& ids,
                            const std::vector<const void*>& data,
                            const std::vector<size_t>& sizes,
                            const std::vector<bool>& is_prefix = {});
    bool get(const BlockId& id, uint8_t* out, size_t* out_size);
    bool get_device(const BlockId& id, void* out, size_t out_capacity, size_t* out_size);


    bool get_device_async(const BlockId& id, void* out, size_t out_capacity,
...
public:
    ...
    bool available(const BlockId& id) const;
    std::vector<uint8_t> available_batch(const std::vector<BlockId>& ids) const;


    size_t available_prefix(const std::vector<BlockId>& ids) const;


    void pin_promised(const std::vector<BlockId>& ids, double seconds);
    void unpin_promised(const std::vector<BlockId>& ids);
    size_t pinned_blocks() const;


    size_t prefetch_batch(const std::vector<BlockId>& ids);
    size_t prefetch_hits() const { return prefetch_hits_.load(); }
    ...
    size_t get_device_batch(const std::vector<BlockId>& ids,
...
private:
    ...
    std::atomic<size_t> prefetch_skipped_full_{0};
    std::atomic<size_t> prefix_blocks_protected_{0};
    std::atomic<size_t> promotions_to_local_{0};
    std::atomic<size_t> prefetched_blocks_{0};
    std::atomic<size_t> prefetch_hits_{0};
    ...
    std::unordered_map<int, void*> transfer_streams_;

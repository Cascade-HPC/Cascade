...
class ShareGPTWorkload:
    """
    Generate inference requests from ShareGPT dataset.

    ShareGPT structure: list of conversations, each with alternating
    human/gpt turns. We use the system prompt + previous turns as
    the shared prefix, and the current human turn as the new portion.
    """
    ...
    def set_prefix_length(self, target_tokens: int, tokenizer=None):
        """Set system prompt length to approximately target_tokens."""
    ...
    def load(self, max_conversations: int = 200) -> int:
        """Load ShareGPT dataset. Returns number of conversations loaded."""
    ...
    def _generate_synthetic(self, num_conversations: int):
        """Generate synthetic multi-turn conversations mimicking ShareGPT."""
        topics = [
    ...
    def generate_requests(
        ...
        """
        Generate inference requests simulating multi-user sessions.

        Multiple sessions share the same system prompt prefix.
        Within each session, later turns share previous conversation history.
        This creates realistic prefix overlap patterns (80-95% in ShareGPT).
        """
        requests = []
        request_id = 0

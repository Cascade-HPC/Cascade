...
@dataclass
class DecodeResult:
    """Result of a decode (generation) operation."""
...
class InferenceEngine:
    """
    Manages model loading and inference for KV cache benchmarking.

    Supports two modes:
    - prefill(): Run full forward pass, return past_key_values
    - decode(): Generate tokens using provided past_key_values
    """
    ...
    def tokenize(self, text: str) -> torch.Tensor:
        """Tokenize input text, return input_ids on model device."""
    ...
    def prefill(self, input_ids: torch.Tensor) -> PrefillResult:
        """
        Run prefill: full forward pass over input tokens.
        Returns past_key_values (the KV cache).
        """
    ...
    def decode(
        ...
        """
        Run autoregressive decode using cached KV.

        Args:
            input_ids: Last token(s) to start generation from
            past_key_values: Cached KV from prefill or Cascade
    ...
    def prefill_and_decode(
        ...
        """Full inference: prefill + decode. Convenience method."""
